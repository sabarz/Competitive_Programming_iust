...
int main ()
{
    ...
    if ((fshm = shm_open ("/posix-shared-mem-example", O_RDWR | O_CREAT | O_EXCL, 0660)) == -1)
    {
        perror ("shm-open");
        exit (1);
    }
    if ((memory_ptr = mmap (NULL, sizeof (struct shared_memory), PROT_READ | PROT_WRITE, MAP_SHARED, fd_shm, 0)) == MAP_FAILED)
    {
        perror ("mmap");
        exit (1);
    }  
    if ((flog = open ("/tmp/example.log", O_CREAT | O_WRONLY | O_APPEND | O_SYNC, 0666)) == -1)
    {
        perror ("fopen");
        exit (1);
    }
